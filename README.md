...
1. Gere uma `SECRET_KEY` nova e exclusiva para produção (não reaproveite a de desenvolvimento).
2. No `.env` de produção: `DEBUG=False` e `ALLOWED_HOSTS` incluindo o domínio real (`seuusuario.pythonanywhere.com`).
3. Rode `python manage.py migrate` no servidor para criar o `db.sqlite3` de produção.
4. Nunca versione `.env` nem `db.sqlite3` em um repositório público.
5. Configure o mapeamento de arquivos estáticos (`/static/`) apontando para a pasta `static/` do projeto no painel "Web" do PythonAnywhere.

## Ferramentas de apoio (não fazem parte do deploy)

`cloudflared.exe` + `tunnel.log`/`server.log` na raiz servem só para expor o `runserver` local via túnel Cloudflare (testar no celular, por exemplo) — não é uma forma de deploy.
